"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PartiesService = void 0;
const common_1 = require("@nestjs/common");
const constant_1 = require("../../resources/constant");
const typeorm_1 = require("typeorm");
let PartiesService = class PartiesService {
    constructor(partiesRepository) {
        this.partiesRepository = partiesRepository;
    }
    create(createPartyInput) {
        const newItem = this.partiesRepository.create(createPartyInput);
        return this.partiesRepository.save(newItem);
    }
    findAll() {
        return this.partiesRepository.find();
    }
    findOne(id) {
        return this.partiesRepository.findOne(id);
    }
    update(id, updatePartyInput) {
        return this.partiesRepository.save(Object.assign(Object.assign({}, updatePartyInput), { id }));
    }
    async remove(id) {
        const removeItem = await this.partiesRepository.findOne(id);
        return this.partiesRepository.remove(removeItem);
    }
};
PartiesService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)(constant_1.PARTIES_REPOSITORY)),
    __metadata("design:paramtypes", [typeorm_1.Repository])
], PartiesService);
exports.PartiesService = PartiesService;
//# sourceMappingURL=parties.service.js.map